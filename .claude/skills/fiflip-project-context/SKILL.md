...
React/Vite (Vercel) ──POST /api/leads/*──> Spring Boot (Railway)
                                                │
                                    LeadProcessingService (synchronous)
                                                │
                              ┌─────────────────┼─────────────────┐
                              ▼                                   ▼
...
`LeadController` calls `LeadProcessingService` synchronously, which:
...
2. Fires **two** Meta Conversions API events (`MetaConversionsService`):
   ...
   - A per-intent custom event, so separate retargeting audiences can be
     ...
     - Renovation `tipo` → `LeadVenderMasCaro` / `LeadListoParaMudarte` /
       ...
       `LeadProcessingService.renovationCustomEventName`).
...
1. **Spring Boot 4 renamed starter artifacts.** It's not
   `spring-boot-starter-web`, it's `spring-boot-starter-webmvc`. Don't
...
2. **Never add `spring-boot-starter-mail` / use SMTP on Railway.** Railway
...
3. **Meta's Custom Audience / Custom Conversion creation UI won't list a
...
4. **Railway's Postgres addon doesn't auto-inject its vars into other
...
5. **New `NOT NULL` columns on `Project` will break the live deploy** if
...
6. **Don't push backend changes to `main` speculatively** when they depend
   ...
   *whole* service — including the parts that already work (email) — if
   the new code can't start. Build and verify locally
...
7. **Vercel's bot-protection interstitial ("Vercel Security Checkpoint")**
...
8. **Adding a value to a Java enum used with `@Enumerated(EnumType.STRING)`
